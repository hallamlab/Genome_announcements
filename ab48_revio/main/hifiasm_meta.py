...
data=Path(f"/arc/project/{SLURM_ACCOUNT}/pwy_group/data/kalen/revio_wgs")
input_raw = [
    ((data/"ABC-240403_KD.fastq.gz"), "sequences::long_reads", dict(parity="single", length_class="long")),
    ((data/"ABP-230518_KD.fastq.gz"), "sequences::long_reads", dict(parity="single", length_class="long")),
]
...
for n, p in [
        ("sequences::hifiasm_meta_assembly",            set()),
        ("sequences::assembly_stats",               {"sequences::hifiasm_meta_assembly"}),
        ("sequences::assembly_per_bp_coverage",     {"sequences::hifiasm_meta_assembly"}),
        ("sequences::assembly_per_contig_coverage", {"sequences::hifiasm_meta_assembly"}),
    ]:
    targets.Add(n, p)
...
smith.StageWorkflow(task, on_exist="clear", verify_external_paths=True)
...
smith.RunWorkflow(
    ...
    resource_overrides={
        ...
        #     cpus=5,
        # ),
        transforms[1]["hifi/hifiasm_meta.py"]: Resources(
            memory=Size.GB(64),
        )
    }
)
